...
> **supersede 注記(2026-07-13)**: 決定事項テーブルの L18「core 専用リポジトリ | 作らない」は
> `2026-07-13-usage-limits-core-spec.md` により supersede された。L18/L17 が前提としていた
> 「engine.ts は複製後に同期機構なしで独立進化(分岐容認)」は撤回し、認証・キャッシュ・usage API
> 呼び出し・percent 計算・backoff の不変ロジック(invariant-core)を script 方式で vendored core
> として同期する設計に置き換える。ただし L18 の rationale(「ユーザーが導入を意識するのはプラグイン
> 2 つだけであるべき」)自体は保持しており、新たな core 専用リポジトリは作らない — 正典は
> tmux-usage-limits リポジトリ内に置き、herdr-usage-limits へは script で同期するのみ。また
> L17 の rendering 分岐容認の意図(herdr が将来 ANSI 直接出力へ進化できること)も、新 spec の
> core/rendering 分割設計の中で明示的に保持している。詳細は新 spec を参照。
