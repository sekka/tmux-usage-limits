...
> **supersede 注記(2026-07-13)**: 初版の決定事項テーブルが前提としていた
> 「engine.ts は複製後に同期機構なしで独立進化(分岐容認)」は
> `2026-07-13-usage-limits-core-spec.md` により撤回された。認証・キャッシュ・usage API
> 呼び出し・percent 計算・backoff の不変ロジック(invariant-core)は、tmux-usage-limits を
> 正典として herdr-usage-limits へ `scripts/sync-core.sh` で同期する。ただし「ユーザーが
> 導入を意識するのはプラグイン 2 つだけであるべき」という意図は保持し、新たな core 専用
> リポジトリは作らない。rendering 層は各リポジトリで分岐可能なまま維持する。
...
| 項目                  | 決定                                                                                     | 根拠                                                                                                                                             |
| --------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| トポロジー            | 2 リポジトリとして独立配布。core は tmux を正典として herdr へ script 同期する             | ユーザーが導入する対象は 2 リポジトリのまま。共有 npm/core 専用リポジトリは作らない                                                              |
| engine.ts             | rendering 層は各リポジトリで保持し、core は `src/usage-limits-core.ts` として同期する     | herdr 側は将来 tmux 書式を捨て ANSI 直接出力に進化できる。不変ロジックだけを同期し、描画差分は許容する                                           |
| core 専用リポジトリ   | 作らない。正典は `tmux-usage-limits/src/usage-limits-core.ts`                             | ユーザーが導入を意識するのはプラグイン 2 つだけであるべき                                                                                        |
...
1. herdr: `herdr plugin uninstall dotfiles.usage-limits` → `herdr plugin install sekka/herdr-usage-limits --ref v1.0.0`(または検証済み commit SHA) → `herdr plugin action invoke start-title-daemon --plugin dotfiles.usage-limits`。キーバインドは ID 維持のため無変更
...
2. **herdr 本番切り替え**: link を unlink → uninstall/install(`herdr plugin install sekka/herdr-usage-limits --ref v1.0.0` または検証済み commit SHA)→ 実描画確認
3. **tmux 専用化**: このリポジトリで削除・README・release config・verify manifest 改修 → ローカルレビューゲート(agmsg で codex peer review、`coderabbit review --base master --agent --type all`、不可時は独立 Opus reviewer-judgment)→ ユーザーのマージ判断
...
- 両リポジトリのローカルレビューゲートを通過。手順は (1) agmsg で codex peer review、(2) `coderabbit review --base master --agent --type all` の妥当な指摘を修正、(3) いずれか不可なら独立 Opus reviewer-judgment、(4) 両レビューまたはフォールバック完了後にのみ "local review passed" と報告
